...
ggplot(
    data=new_df, aes(x=City, y=Count, fill=Industry))+
    geom_bar(position = "stack", stat="identity")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
```
...
plot(x = df_GI[df_GI$Valuation.Billion > 5, ]$Investors.Count,
     y = df_GI[df_GI$Valuation.Billion > 5, ]$Valuation.Billion,
        pch = 19,
...
plot(x = df_LI[df_LI$Valuation.Billion > 5, ]$Investors.Count,
     y = df_LI[df_LI$Valuation.Billion > 5, ]$Valuation.Billion,
        pch = 19,
