setwd("C:/Users/weilu/OneDrive - purdue.edu/Desktop/Sampling/Homework 5")
...
library(haven)
library(dplyr)
###########################Question 1 Comparing SRS and Stratified Sampling using the Object data
#You can read SAS file directly using the haven package. 
shapedata<-read_sas('shapespop.sas7bdat')
...
index_STR<-strata(shapedata_sorted,
              stratanames=c("shape"),
              size=c(50,150), #This is what we computed previously
              method="srswor")
...
###########################Question 2 Comparing SRS and Stratified Sampling using the Baseball data
#You can read SAS file directly using the haven package. 
baseballdata<-read_sas('baseball.sas7bdat')
names(baseballdata)
baseballdata<-as.data.frame(baseballdata)
...
# Calculate mean and CI for gray objects
