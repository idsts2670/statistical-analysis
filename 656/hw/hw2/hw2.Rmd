...
As we can see the output of the model, the parameter `x1_sq` and `x1_x2` are not significant in the model.\
I will remove these parameters from the model and run the model again.
...
```{r}
X <- df[, c("x1", "x2", "x2_sq", "offset")]
y <- df$y

# compile the model
poissonreg_data2 <- list(N = nrow(X), p = ncol(X), pr_sd = 100, x = X, y = y)
