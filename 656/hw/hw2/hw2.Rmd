...
data {
  ...
  real<lower=0> alpha; // Shape parameter for Inverse-Gamma
  real<lower=0> beta; // Scale parameter for Inverse-Gamma
...
parameters {
  ...
  vector[p] Rho; // Parameters to estimate
...
transformed parameters {
  ...
  vector[N] mu = x * Rho; // Linear predictor
}

// The actual Bayesian model goes here
// I set normal dist as a prior for Rho
model {
  
  // priors
  Rho ~ normal(0, pr_sd); // Note: Rho is p-dim
...
# build the model before sampling
poissonreg_normal <- stan_model(model_code = linreg_normal_code)
```
...
```{r}
# compile the model
reg1_data <- list(N = nrow(X), p = ncol(X),  x = X, y = y, alpha = 1, beta = 1, pr_sd = 100)
...
posterior_samples <- as.data.frame(rstan::extract(reg_fit, permuted = TRUE))

# Extract the summary of the fit
fit_summary <- summary(reg_fit)$summary
# extract them individually
rho_1_mean <- fit_summary["Rho[1]", "mean"]
rho_2_mean <- fit_summary["Rho[2]", "mean"]
rho_3_mean <- fit_summary["Rho[3]", "mean"]
rho_4_mean <- fit_summary["Rho[4]", "mean"]
rho_5_mean <- fit_summary["Rho[5]", "mean"]
rho_6_mean <- fit_summary["Rho[6]", "mean"]

# Define a grid of concentration sequences for the two modulators
modulator_A_seq <- seq(0, 80, length.out = 100)
modulator_B_seq <- seq(0, 30, length.out = 100)

# Create a grid of concentration pairs for the two modulators
grid <- expand.grid(modulator_A = modulator_A_seq, modulator_B = modulator_B_seq)

# Create the function for it
mean_predictives <- function(modulator_A, modulator_B) {
  # Compute the predictive mean using the model equation and the posterior samples
  mean_predictive <- mean(rho_1_mean +
                          rho_2_mean * modulator_A +
                          rho_3_mean * modulator_B +
                          rho_4_mean * modulator_A^2 +
                          rho_5_mean * modulator_B^2 +
                          rho_6_mean * (modulator_A * modulator_B)
                          )
  return(mean_predictive)
  }

reg_lik <- outer(modulator_A_seq, modulator_B_seq, mean_predictives)
# # Create the function for it
# mean_predictives <- function(row) {
...
predictive_means <- apply(grid, 1, mean_predictives)
