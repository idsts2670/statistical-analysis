...
I have checked the histogram of the posterior distribution of the parameters, and it seems that `x1_sq` and `x1_x2` are not significant in the model.\
Therefore, we try to compare the model with and without `x1_sq` and `x1_x2` to see which model is better.
```{r}
...
Apparently, without `x1_sq` and `x1_x2`, the posterior distribution of the parameters become more significant.\
```{r fig.show="hold", out.width="50%"}
...
Here, I created a contour plot of the posterior predictive mean of conversion as a function of concentration settings to visualize the relationship between the modulators and conversion.
