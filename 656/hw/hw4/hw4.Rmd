...
probitVB <- function(X, y, max.iter = 1000, burn.in = 0) {
  ...
  sigma_beta_q_history <- matrix(NA, nrow = max.iter, ncol = d)
  ...
  for (iter in 1:max.iter) {
    ...
    Z[iter, ] <- E_z
    mu_beta <- sigma_beta_q %*% (t(X) %*% E_z)

    # M-step: Update q(beta) based on current q(z_i)'s
    sigma_beta_q_inv <- solve(sigma_beta) + xtx
    sigma_beta_q <- solve(sigma_beta_q_inv)
    mu_beta <- sigma_beta_q %*% (t(X) %*% E_z) # update mean

    # store history
    mu_beta_history[iter, ] <- mu_beta
    sigma_beta_q_history[iter, ] <- sigma_beta_q
    Z[iter, ] <- E_z

  }

  new_list <- list(
    beta = mu_beta[burn.in:max.iter, ],
    sigma_beta = sigma_beta_q[burn.in:max.iter, ],
    iterations = max.iter,
    sigma_beta_history = sigma_beta_q_history[burn.in:max.iter, ],
...
posterior.syn <- probitVB(X, y, max.iter = 5000)
posterior.beta <- posterior.syn[[1]]
p1 <- ggplot() + 
  geom_point(aes(c(1:5000), posterior.beta[,1])) +
  labs(x = "Iteration", y = "Values of Beta Parameters"
  )
p2 <- ggplot() + geom_point(aes(c(1:5000), posterior.beta[,2]))
...
# Extract beta_history from the results
beta_history <- results$beta_history

# Number of parameters in beta
num_params <- ncol(beta_history)

# Create a plot
plot(NULL, xlim = c(1, nrow(beta_history)), ylim = range(beta_history, na.rm = TRUE),
     xlab = "Iteration", ylab = "Values of Beta Parameters", type = "n")

# Add lines for each parameter
colors <- rainbow(num_params)
for (i in 1:num_params) {
  lines(beta_history[, i], col = colors[i])
}

# Adding a legend to distinguish the parameters
legend("topright", legend = paste0("Beta ", 1:num_params), col = colors, lty = 1, cex = 0.8)
