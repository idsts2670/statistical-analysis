    ...
    for (i in 1:length(y)) {
        for (k in 1:length(y[[i]])) {
            ...
            ifelse(
                ...
                # And if "i"th value in "Investor.label" column is blank, simply insert "1",
                ifelse(
                    is.null(x$Investor.label[i]),
                    x$Investor.label[i] <- 1,
                    # if "i"th value in "Investor.label" column = "1", keep it, if = "0", replace it with "1"
                    ifelse(
                        x$Investor.label[i] == 1,
                        x$Investor.label[i],
                        x$Investor.label[i] <- 1)
                    # if "k"th investor in "i"th list in dataframe is not in top_investors$investors, insert "0"
