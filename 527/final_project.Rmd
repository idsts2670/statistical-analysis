...
```{r}
boxplot(data=df,
        df$Date.Joined_in_months,
        main="Time Companies' Became Unicorn Companies",
        ylab="Month Joined")
boxplot(data=df,
        Date.Joined_in_months~Industry,
        main="Time of Each Industry Became Unicorn Companies",
        xlab = "Industry",
        ylab = "Month Joined",
        col="orange",
        border="brown")
...
df3 <- df2 %>%
        mutate(Investor.label = ifelse(
                Select.Investors %in% top_investors, 1, 0))
head(df2)
typeof(df2$Select.Investors)
class(df2$Select.Investors)
...
investor.fun <- function(x) {
    ...
    for (i in 1:length(y)) {
        for (k in 1:length(y[[i]])) {
            # if "k"th investor in "i"th list in dataframe is in top_investors$investors
            ifelse(y[[i]][k] %in% top_investors$investors,
                # And if "i"th value in Investor.label column
                ifelse(is.null(x$Investor.label[i]), x$Investor.label[i] <- 1,
                    ifelse(x$Investor.label[i] == 1, x$Investor.label[i], x$Investor.label[i] <- 1)
                ), x$Investor.label[i] <- 0)
        }
    }
    return(x)
}
length(df2$Select.Investors)
tmp <- investor.fun(df2)
head(tmp)
df2$Select.Investors[[966]][3] %in% top_investors$investors
length(df2$Select.Investors[[966]])
FUN <- function(x){
    for (i in length(x)) {
        mutate(Investor.label = ifelse(
            for (k in length(x[[i]])) {
                    Select.Investors[[i]] %in% top_investors, 1, 0))
                }
            }
        }
...
apply(df2$Select.Investors, 1, FUN)
```